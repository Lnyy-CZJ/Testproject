...
async function mockAdminDesignRoutes(page: Page) {
...
test.describe('design system admin pages', () => {
  test('缺陷列表移除 hero 并保留动作区与筛选区', async ({ page }) => {
    await seedAuth(page, { lastProjectId: '1' });
    await mockAdminDesignRoutes(page);

    await page.goto('/projects/1/defects');

    await expect(page.getByTestId('defect-list-hero')).toHaveCount(0);
    await expect(page.locator('.page-action-bar')).toBeVisible();
    await expect(page.getByTestId('defect-filter-rail')).toBeVisible();
  });

  test('平台治理页面移除 hero', async ({ page }) => {
    await seedAuth(page);
    await mockAdminDesignRoutes(page);
    ...
    page.on('console', (msg) => {
      ...
      if (warningPatterns.some((pattern) => text.includes(pattern))) {
        warnings.push({ url: currentUrl, text });
      }
    });

    const cases = [
      ['/users', 'user-management-hero', true],
      ['/audit-logs', 'audit-log-hero', true],
      ['/ai-catalog', 'ai-catalog-hero', true],
      ['/platform-credentials', 'platform-credentials-hero', true],
      ['/platform-settings', 'platform-settings-hero', true],
    ] as const;
