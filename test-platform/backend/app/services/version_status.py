...
from app.models.configuration import ConfigActivation
...
def collect_snapshot(database: Session, settings: Settings) -> dict[str, Any]:
    ...
    return {
        ...
        "database": {"alembic_revision": revision},
        "config_releases": {
...
def compare_component(
    ...
    if prod and prod_expected:
        for field in ("version", "revision", "digest"):
            expected = prod_expected.get(field)
            if expected and prod.get(field) != expected:
                issues.append("环境漂移")
                break
    if dev and dev.get("version") == (prod or {}).get("version"):
        if dev.get("revision") not in {None, "unknown", (prod or {}).get("revision")}:
            issues.append("构建不一致")
