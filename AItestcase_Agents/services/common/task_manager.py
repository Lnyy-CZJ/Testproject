...
class TaskManager:
    ...
    def _runner_environment(self, snapshot: dict[str, Any], task_id: str) -> dict[str, str]:
        ...
        environment = {
            ...
            "LLM_MODEL": str(normal.get("LLM_MODEL", "deepseek-v4-flash")),
            "base_url": str(normal.get("LLM_BASE_URL", "https://dashscope.aliyuncs.com/compatible-mode/v1")),
            "DASHSCOPE_API_KEY": str(secrets.get("LLM_API_KEY", "")),
            "CASE_GENERATION_BATCH_SIZE": str(normal.get("CASE_GENERATION_BATCH_SIZE", 5)),
            "USE_COVERAGE_MATRIX": str(normal.get("COVERAGE_MATRIX_ENABLED", True)).lower(),
            ...
            "REVIEW_AI_MAX_CONTEXT_POINTS": str(normal.get("REVIEW_AI_MAX_CONTEXT_POINTS", 500)),
            "REVIEW_AI_MAX_SUGGESTIONS": str(normal.get("REVIEW_AI_MAX_SUGGESTIONS", 200)),
            "ONLINE_CASE_REVIEW_ENABLED": str(normal.get("ONLINE_CASE_REVIEW_ENABLED", False)).lower(),
            "CASE_REVIEW_AI_MAX_CONTEXT_CASES": str(normal.get("CASE_REVIEW_AI_MAX_CONTEXT_CASES", 300)),
            "CASE_REVIEW_AI_MAX_CONTEXT_POINTS": str(normal.get("CASE_REVIEW_AI_MAX_CONTEXT_POINTS", 300)),
            "CASE_REVIEW_AI_MAX_SUGGESTIONS": str(normal.get("CASE_REVIEW_AI_MAX_SUGGESTIONS", 100)),
            ...
            "REQUIREMENT_DECOMPOSITION_CONFIG": str(self.project_root / "requirement_decomposition.yaml"),
        }
    ...
    def _run_task(self, task_id: str) -> None:
        ...
        try:
            ...
            if self.require_llm_config and not (snapshot.get("secrets", {}) or {}).get("LLM_API_KEY"):
                raise ServiceError(503, "CONFIG_NOT_READY", "LLM API Key 尚未配置")
            model_name = str(normal.get("LLM_MODEL", "deepseek-v4-flash"))
