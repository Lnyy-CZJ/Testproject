    ...
    *,
    ...
    def readiness():
        ...
        try:
            ...
            return jsonify({
                ...
                "environment": settings.runtime_environment,
                "online_review_enabled": bool(normal.get("ONLINE_REVIEW_ENABLED", False)) if settings.agent_type == "functional" else False,
    ...
    def create_task():
        ...
        try:
            ...
            if has_test_points_upload and (settings.agent_type != "functional" or operation != "generate_test_cases"):
            ...
            if upload and upload.filename:
                allowed = {".json"} if is_test_points else (FUNCTIONAL_EXTENSIONS if settings.agent_type == "functional" else API_EXTENSIONS)
    ...
    def task_detail(task_id: str):
        ...
        payload["can_resume"] = record.get("status") == "waiting_review" and settings.agent_type == "functional"
    ...
    def get_review(task_id: str):
        """返回在线 Review 数据；API 智能体与关闭开关均失败关闭。"""
