...
(() => {
  ...
  function render() { renderSummary(); renderIssues(); renderTable(); updateControls(); }
  ...
  function applyServerReview(result) { state.points = withKeys(result.points); state.original = result.original_points || state.original; state.revision = result.revision; state.sha256 = result.sha256; state.validation = result.validation; state.diff = result.diff_summary; }
