...
@pytest.mark.parametrize("agent_type,filename", [("functional", "requirement.md")])
...
def test_artifact_expired_and_missing_identity_fail_closed(tmp_path: Path) -> None:
    ...
    client, app = make_client(tmp_path, "functional")
    assert client.get("/functional-test-agent/").status_code == 401
    created = client.post(
        "/functional-test-agent/api/v1/tasks", headers=headers(),
        data={
            "operation": "generate_test_points", "project_name": "A", "module_name": "B", "environment": "dev",
            "document_file": (io.BytesIO(b"# requirement"), "requirement.md"),
        }, content_type="multipart/form-data",
    ).get_json()
    record = app.extensions["task_store"].load(created["id"])
    record["artifacts_expired"] = True
    app.extensions["task_store"].save(record)
    response = client.get(f"/functional-test-agent/api/v1/tasks/{created['id']}/artifacts", headers=headers())
...
def test_admin_task_list_survives_schema_incompatible_records(tmp_path: Path) -> None:
    ...
    created = client.post(
        "/functional-test-agent/api/v1/tasks",
        headers=headers(),
        data={
            "operation": "generate_test_points", "project_name": "项目 A", "module_name": "登录", "environment": "dev",
            "document_file": (io.BytesIO(b"# requirement"), "requirement.md"),
        },
        content_type="multipart/form-data",
    )
    ...
    assert page.status_code == 200
    listing = client.get("/functional-test-agent/api/v1/tasks", headers=admin_headers)
