...
- 原定时规则为每天北京时间凌晨 2 点所在小时执行，当前已暂停。
- Python 虚拟环境和依赖安装。
...
| 分支 | `*/dev` |
| Script Path | `api-autotest/Jenkinsfile` |
...
当前状态：已按用户要求暂停，`Jenkinsfile` 不声明 `triggers`/`cron`，Jenkins 任务中也不保留 TimerTrigger。
...
post {
    always {
        junit(
            ...
            testResults: 'api-autotest/reports/junit*.xml'
        )
        ...
        allure(
            ...
            results: [[path: 'api-autotest/allure-results']]
        )

        archiveArtifacts(
            allowEmptyArchive: true,
            artifacts: 'api-autotest/logs/**/*,api-autotest/allure-results/**/*'
        )
    }
}
```
...
3. 提交到 `dev`。
4. 推送远程仓库。
5. 确认远程存在 `api-autotest/Jenkinsfile`。
...
- 原定时规则：`TZ=Asia/Shanghai`、`H 2 * * *`
- `2026-08-01` 至 `2026-08-05` 的自然定时构建 `#10`–`#14` 均为 `SUCCESS`，且均生成 6 条 Allure 结果。
- 当前状态：自动触发已暂停，手动参数化构建不受影响。
...
| 定时构建验收 | Jenkins | `#10`–`#14` 已验收，当前已暂停 |
