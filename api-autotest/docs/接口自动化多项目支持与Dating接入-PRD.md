...
| 实施边界 | 测试开发平台负责运行作用域、配置发布、Secret/Credential、统一身份与入口；`api-autotest` 负责 Project Manifest、API/Case/Flow/Scenario/Fixture、执行引擎及接口自动化 Web 业务能力 |

### 1.1 文档目的

本文档定义 `api-autotest` 从 Truthy 单项目接口自动化框架升级为通用多项目接口自动化工具的产品需求，并明确 Dating 项目的首期接入范围。

本期需求包含接口自动化 Web 界面的开发，不是仅改造 CLI 或执行引擎。页面、页面调用的任务/目录/运行前校验接口、任务状态与报告逻辑均由 `api-autotest` 交付；测试开发平台负责展示 `api-autotest` 入口，并作为运行配置控制面，统一管理项目运行作用域、接口环境、配置 Release、Secret/Credential、身份权限与配置快照下发。平台不复制接口自动化页面、用例资产、执行逻辑或任务状态机，工具也不再提供第二套可编辑的项目运行配置。
...
- `api-autotest` 当前 V1.3 接口自动化框架；
- `api-autotest` 当前测试开发平台接入能力；
...
8. 以 `api-autotest` 自带 Web 工具页承载完整操作体验，并通过测试开发平台入口访问；
9. 将平台确立为可变运行配置唯一真源，将 `api-autotest` 确立为测试资产唯一真源，彻底消除双配置和静默回退。
...
| S-13 | Web 页面与任务业务接口均由 `api-autotest` 提供；平台提供通用配置控制面但不形成第二套接口自动化页面、用例库或任务实现 |
...
- 在 `api-autotest` 内完成概览、项目切换、执行任务、用例库、任务记录和任务详情页面，并提供内联运行前校验；
- Truthy 项目包迁移与兼容入口；
- Dating 首期项目包；
...
- 在 `api-autotest` 中建立与平台平行的项目运行配置中心，或在线编辑平台拥有的 Gateway、`comm`、超时、轮询、Secret/Credential；
...
`api-autotest` 是接口自动化测试资产、执行与 Web 业务能力所有者；测试开发平台是统一入口、权限和运行配置控制面。职责按下表划分：

| 能力 | `api-autotest` | 测试开发平台 |
| --- | --- | --- |
...
| Project Manifest、配置键/Profile 契约 | `api-autotest/projects/` | 校验平台快照是否满足执行需求，不保存运行值 |
| API、Case、Flow、Scenario、Fixture | `api-autotest/projects/` | 收集、执行和展示，平台不得复制或在线编辑 |
| 任务、日志、JUnit、Allure | `api-autotest` | 记录 Scope/Release 快照并按项目、任务隔离 |
...
`api-autotest` 提供运行前校验接口，供概览页和任务提交区域内联使用：
...
| Project Manifest、配置契约 | `api-autotest/projects/` | 代码评审与版本发布 | 仅保存键/Profile 契约，不保存值 |
| API、Case、Flow、Scenario、Fixture | `api-autotest/projects/` | 代码评审与版本发布 | 是 |
...
本章定义的接口自动化页面均由 `api-autotest` 的 `web/` 模块实现。测试开发平台提供工具入口、通用配置中心及第 6.7 节定义的控制面能力。工具不提供独立配置页面；配置维护统一在平台完成，工具仅在当前操作位置展示必要的运行前校验结果。
...
- 整个迁移过程不要求把平台配置导出回 `api-autotest` 仓库。
...
1. 用户从测试开发平台的 `api-autotest` 入口进入由 `api-autotest` 提供的 Web 工具页；
...
13. 接口自动化页面、资产、任务 API 和执行业务逻辑位于 `api-autotest`；平台改动归属于通用 Runtime Scope、配置/Secret/Credential 控制面、工具注册、路由和身份能力。
...
- 在 `api-autotest` 建立版本化平台快照消费契约和 fail-closed 模式。
...
- 在 `api-autotest` 内增加项目列表、项目选择器及七类 Web 页面状态；
- 在 `api-autotest` 内扩展任务提交、任务展示、目录、运行前校验和报告接口；
...
- Dating test 首期项目包；
- 项目选择 CLI；
- `api-autotest` 内的多项目 Web 页面、页面业务 API 与任务能力；
...
- [ ] Web 的七类页面和相关业务 API 均在 `api-autotest` 内实现，测试开发平台未形成重复实现；
