...
def _normalize_platform_settings(settings: dict[str, Any]) -> None:
    ...
    # 根级单请求可声明任意命名 target。平台 Definition 使用稳定点分键，运行时
    # 再组装成 GatewayApi 既有的 gateway_targets；项目包因此不保存环境 URL。
    gateway_targets = settings.get("gateway_targets")
    if gateway_targets is None:
        gateway_targets = {}
    if not isinstance(gateway_targets, dict):
        raise ConfigError("平台运行快照 gateway_targets 必须是对象")
    nested_targets = gateway_values.get("targets")
    if isinstance(nested_targets, dict):
        gateway_targets = _deep_merge(gateway_targets, nested_targets)
    target_prefix = "gateway.targets."
    for dotted_name in list(settings):
        if not dotted_name.startswith(target_prefix):
            continue
        suffix = dotted_name[len(target_prefix) :]
        parts = suffix.split(".", 1)
        if len(parts) != 2 or parts[1] not in {"base_url", "path", "method", "headers"}:
            continue
        target_name, field = parts
        target = gateway_targets.setdefault(target_name, {})
        if not isinstance(target, dict):
            raise ConfigError(f"平台 Gateway target {target_name} 必须是对象")
        target[field] = deepcopy(settings.pop(dotted_name))
    for target in gateway_targets.values():
        if isinstance(target, dict) and target.get("base_url") and target.get("path"):
            target.setdefault("method", "POST")
    if gateway_targets:
        settings["gateway_targets"] = gateway_targets
