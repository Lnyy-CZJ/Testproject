...
def test_flow_json_runtime_input_replaces_one_structured_request_object() -> None:
    """结构化消息应作为一个 JSON 对象编辑，不能展开成数十个表单字段。"""

    flow = {"steps": [{"id": "create_evaluation", "api": "Demo"}]}
    default_input = {
        "locale": "en-US",
        "transcript": {
            "schema_version": "dating.transcript.v1",
            "messages": [
                {"message_id": "m1", "speaker": "other", "text": "Hello"},
                {"message_id": "m2", "speaker": "user", "text": "Hi"},
            ],
        },
    }
    scenario = {
        "runtime_inputs": {
            "evaluation_request": {
                ...
                "description": "每次运行可替换完整 JSON 对象",
                "type": "json",
                "required": True,
                "target": {
                    "scope": "flow_step_request",
                    "step_id": "create_evaluation",
                    "path": "$.input",
                },
            }
        },
    ...
    normalized = validate_flow_runtime_inputs(flow, scenario, scope="Flow evaluation")
    flow_case = {
        "id": "evaluation",
        "name": "Evaluation",
        "tags": ["interactive"],
    ...
    replacement = {
        ...
        "transcript": {
            "schema_version": "dating.transcript.v1",
            "messages": [
                {"message_id": "x1", "speaker": "other", "text": "周末见吗？"},
                {"message_id": "x2", "speaker": "user", "text": "可以。"},
            ],
        },
    }

    assert list(normalized) == ["evaluation_request"]
    assert normalized["evaluation_request"]["type"] == "json"
    resolved = apply_runtime_overrides(
        snapshot,
        {"evaluation_request": replacement},
        expected_revision=snapshot["asset_revision"],
        require_revision=True,
    )
    ...
    assert params["input"] == replacement
    assert snapshot["runtime_input_definitions"]["evaluation_request"][
        "default_value"
    ] == default_input
...
@pytest.mark.parametrize(
    ...
    [
        ["JSON 根节点不能是数组"],
        {"transcript": {"messages": [{"text": "{{AUTH_TOKEN}}"}]}},
    ],
)
def test_json_runtime_input_rejects_array_and_nested_template(invalid_value) -> None:
    """JSON 输入仅允许对象，并递归拒绝可能读取运行时 Secret 的模板。"""

    snapshot = _case_snapshot(
        input_type="json",
        default_value={"locale": "en-US", "transcript": {"messages": []}},
    )

    with pytest.raises(RuntimeOverrideError) as caught:
        apply_runtime_overrides(
            snapshot,
            {"editable": invalid_value},
            expected_revision=snapshot["asset_revision"],
            require_revision=True,
        )
...
def test_flow_loader_preserves_only_step_client_request_id_comm(
    multi_project_root: Path,
) -> None:
    """Scenario 仅能声明逐步骤幂等 ID，不能覆盖认证或静态设备字段。"""
    ...
    scenario.write_text(
        original.replace(
            "  get_me:\n    params:",
            "  get_me:\n    comm:\n      client_request_id: \"{{flow_run_id}}-get-me\"\n    params:",
        ),
        encoding="utf-8",
    )

    selected = load_flow_cases(project, "dating_demo_flow")[0]
    assert selected["scenario"]["step_data"]["get_me"]["comm"] == {
        "client_request_id": "{{flow_run_id}}-get-me"
    }
    ...
    with pytest.raises(FlowConfigError, match=r"comm.*auth_token"):
        load_flow_cases(project, "dating_demo_flow")


def test_flow_loader_rejects_continue_value_outside_terminal_values(
    multi_project_root: Path,
) -> None:
    """可继续终态必须属于 terminate_on，避免拼写错误静默改变失败语义。"""

    project = multi_project_root / "projects" / "dating"
    flow = project / "data" / "flows" / "dating_demo_flow.yaml"
    flow.write_text(
...
""",
    ...
    with pytest.raises(FlowConfigError, match="continue_flow_on.*terminate_on"):
        load_flow_cases(project, "dating_demo_flow")
