...
def test_structured_evaluation_flows_publish_exact_platform_requirements(
    project_root: Path,
) -> None:
    """结构化评测只要求自身 Endpoint 与 API Key，不应错误依赖匿名会话。"""
    ...
    for flow_id in (
        ...
        flow = flows[flow_id]
        assert flow["credential_profiles"] == []
        assert flow["required_config_keys"] == [
            "gateway.targets.dating_evaluation.base_url",
            "gateway.targets.dating_evaluation.path",
        ]
        assert flow["required_secret_keys"] == ["DATING_EVALUATION_API_KEY"]
