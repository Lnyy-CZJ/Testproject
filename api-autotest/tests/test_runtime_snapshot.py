...
def test_platform_snapshot_normalizes_named_root_gateway_target(
    tmp_path: Path,
) -> None:
    """评测 Endpoint 应由当前 Scope Release 生成命名 target，不能写死在项目包。"""

    snapshot = _snapshot(
        settings={
            "gateway.base_url": "https://gateway.scope.example",
            "gateway.path": "/dating/gateway/invoke",
            "gateway.comm": {"device_id": "scope-device"},
            "gateway.targets.dating_evaluation.base_url": "http://evaluation.example",
            "gateway.targets.dating_evaluation.path": "/admin/invoke",
            "runtime_variables": {
                "DATING_EVALUATION_API_KEY": "runtime-only-evaluation-key"
            },
        }
    )
    snapshot_path = tmp_path / "snapshot.json"
    ...
    assert loaded["gateway_targets"]["dating_evaluation"] == {
        "base_url": "http://evaluation.example",
        "path": "/admin/invoke",
        "method": "POST",
    }
    assert loaded["runtime_variables"]["DATING_EVALUATION_API_KEY"] == (
        "runtime-only-evaluation-key"
    )
