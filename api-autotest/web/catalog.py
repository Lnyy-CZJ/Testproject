...
def _api_runtime_requirements(definition: dict[str, Any]) -> tuple[list[str], list[str]]:
    """从 API 传输契约提取选中资产才需要的平台普通配置与 Secret。

    标准客户端 Gateway 仍使用项目 Manifest 的通用键；只有根级单请求的命名
    target 采用 ``gateway.targets.<target>`` 约定，避免给其他项目硬编码 Dating
    分支。Bearer 变量只向浏览器公开键名，不公开任何 Secret 值。
    """
    ...
    if transport.get("envelope") == "root_single" and target not in (None, "default"):
        config_keys = [
            f"gateway.targets.{target}.base_url",
            f"gateway.targets.{target}.path",
        ]
    bearer_variable = transport.get("bearer_token_variable")
    secret_keys = [str(bearer_variable)] if bearer_variable else []
...
def build_catalog(
    ...
    for api_id, definition in definitions.items():
        ...
        required_config_keys, required_secret_keys = _api_runtime_requirements(
            definition
        )
        apis.append(
            {
    ...
    try:
        for single_case in load_single_cases(asset_root):
            case_definition = definitions.get(single_case["api_id"]) or {}
            required_config_keys, required_secret_keys = _api_runtime_requirements(
                case_definition
            )
    ...
    for flow_path in sorted(flows_directory.glob("*.yaml")):
        ...
        for flow_case in flow_cases:
            ...
            required_config_keys = sorted({
                key
                for definition in flow_definitions.values()
                for key in _api_runtime_requirements(definition)[0]
            })
            required_secret_keys = sorted({
                key
                for definition in flow_definitions.values()
                for key in _api_runtime_requirements(definition)[1]
            })
