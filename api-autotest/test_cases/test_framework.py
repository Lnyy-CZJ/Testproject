...
def test_build_payload_uses_flow_step_client_request_id_only_in_comm() -> None:
    """客户端 Flow 可固定幂等键，但该字段只能出现在 Gateway ``comm``。

    最新 Dating 客户端协议不再允许把 ``client_request_id`` 混入业务
    ``params``。逐步骤声明由运行时变量解析，既能支持同一 Flow 的幂等重试，
    也不会开放 auth_token、user_id 等会话字段覆盖能力。
    """

    runtime = RuntimeContext({"flow_run_id": "run-1"})
    settings = {
        "comm": {
            "device_id": "dating-device",
            "platform": "ios",
    ...
    case = {
        "comm": {"client_request_id": "{{flow_run_id}}-reply"},
        "request": {
    ...
    payload = build_payload(settings, case, runtime)

    assert payload["comm"]["client_request_id"] == "run-1-reply"
    assert "client_request_id" not in payload["requests"][0]["params"]


def test_build_payload_supports_root_single_request_envelope() -> None:
    """内部评测入口应发送根级单请求，且引擎字段不能被交互 JSON 覆盖。"""
    ...
    case = {
        "request": {
            ...
            "params": {
                "input": {
                    "case_id": "browser-attempt",
                    "locale": "zh-CN",
                    "transcript": {"schema_version": "dating.transcript.v1"},
                },
                "case_id": "reply-structured-default",
                "run_id": "run-1",
                "client_request_id": "run-1-reply",
            },
        },
        "transport": {
            "target": "dating_evaluation",
            "envelope": "root_single",
            "params_container": "input",
...
def test_gateway_api_adds_evaluation_bearer_without_logging_contract_leak() -> None:
    """评测 API Key 只进入真实请求，并声明为 HttpClient 定向敏感 Header。"""

    class RecordingClient:
        """记录 GatewayApi 交给 HTTP 层的完整调用参数。"""
        ...
        def post_json(self, **kwargs: object) -> FakeResponse:
            self.call = kwargs
            return FakeResponse(200, {"code": 0, "message": "ok", "responses": []})
    ...
    gateway = GatewayApi(
        {
            ...
            "gateway_targets": {
                "dating_evaluation": {
                    "base_url": "http://evaluation.example",
                    "method": "POST",
                    "path": "/admin/invoke",
                }
            },
        ...
        runtime_context=RuntimeContext({"DATING_EVALUATION_API_KEY": "eval-secret"}),
    )
    case = {
        "request": {
            "service_name": "tool.dating.internal.DatingEvaluationService",
    ...
    assert client.call["url"] == "http://evaluation.example/admin/invoke"
    assert client.call["headers"] == {"Authorization": "Bearer eval-secret"}
    assert client.call["sensitive_headers"] == {"Authorization"}
...
def test_http_client_masks_only_explicit_sensitive_headers(
    caplog: pytest.LogCaptureFixture,
) -> None:
    """其他日志保持原文时，评测 API Key 仍必须从日志和报告附件移除。"""

    class RecordingSession:
        """保存真实网络参数，证明隐藏只影响诊断副本。"""
    ...
    with caplog.at_level(logging.INFO):
        HttpClient(session=session).post_json(
            "http://evaluation.example/admin/invoke",
            {"Authorization": "Bearer evaluation-secret", "X-Debug": "visible"},
    ...
    assert "evaluation-secret" not in caplog.text
    assert "[REDACTED: evaluation API key]" in caplog.text
