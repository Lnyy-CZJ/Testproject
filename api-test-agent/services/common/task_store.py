...
class TaskStore:
    ...
    def _is_visible_to(record: dict[str, Any], identity: Any) -> bool:
        """以根任务不变授权快照过滤 own/project/global 可见范围。

        不读取浏览器声称的 owner/project 或旧 ``task.view.all``；快照缺失的
        历史记录按不可见处理，避免迁移期间的未知资源泄露。
        """
    ...
    def load_visible(self, task_id: str, identity: Any) -> dict[str, Any] | None:
        """读取单根资源；越权与缺失统一返回 None，防止 IDOR 探测。"""
    ...
    def enforce_retention(self, **kwargs: Any) -> dict[str, list[str]]:
        ...
        with self._lock:
            for task_id in plan["expire_artifacts"]:
                ...
                artifact_directories = ["input", "work", "published"]
                if record.get("schema_version") == 2 and record.get("agent_type") == "api":
                    # V2 的版本、Attempt 和 Run 也可能包含脱敏后的业务数据，必须同产物一起过期。
                    artifact_directories.extend(["versions", "attempts", "runs"])
