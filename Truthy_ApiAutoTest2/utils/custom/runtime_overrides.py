...
def _validate_runtime_value(
    ...
    *,
    ...
    if isinstance(value, str) and DYNAMIC_TEMPLATE_PATTERN.search(value):
...
def _normalize_common_definition(
    ...
    *,
    ...
    if input_type not in RUNTIME_INPUT_TYPES:
        raise _target_error(field_scope, "type 必须是 string/integer/number/boolean/enum")
    ...
    *,
    ...
    for key, declaration in declarations.items():
        ...
        if isinstance(default_value, (dict, list)):
            raise _target_error(field_scope, "目标必须是静态叶子值，不能是对象或数组")
