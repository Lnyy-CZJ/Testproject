...
(function () {
  ...
  function safeManagementUrl(url) {
    ...
    try {
      ...
      // 管理入口由平台返回，但仍只接受本站两个已知根路径。拒绝绝对 URL、
      // protocol-relative URL 与其它页面，避免异常响应把用户带到外部站点。
      if (!raw.startsWith("/") || raw.startsWith("//")) return "";
      const parsed = new URL(raw, window.location.origin);
      if (parsed.origin !== window.location.origin) return "";
      if (!["/account/credentials", "/settings/config"].includes(parsed.pathname)) return "";
  ...
  async function initTaskForm() {
    ...
    byId("runtime-input-reset-all").addEventListener("click", () => {
      ...
      renderRuntimeInputs();
      runPreflight();
      const firstControl = byId("runtime-input-fields").querySelector("input, select");
