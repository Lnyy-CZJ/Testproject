    ...
    def invoke(self, case: dict[str, object]) -> _Response:
        ...
        if method == "GetUserPreferences":
            data = {
                ...
                "dating_goal": "serious_relationship" if self.preferences_complete else "",
                "your_voice": "warm_direct" if self.preferences_complete else "",
...
def test_real_multi_image_flow_uploads_each_image_and_keeps_remote_task(
    ...
    def gateway_factory(context):
        gateway = _MultiImageGateway(context)
...
def test_reply_flow_updates_incomplete_preferences_before_ordered_uploads(
    ...
    assert update_params["dating_goal"] == "serious_relationship"
    assert update_params["your_voice"] == "warm_direct"
...
def test_reply_flow_skips_preference_update_when_already_complete(
    ...
    assert gateway.methods[-2:] == ["GetTask", "GetTaskResult"]


def test_reply_flow_failed_terminal_does_not_read_result_or_delete_task(
    tmp_path: Path,
    caplog: pytest.LogCaptureFixture,
) -> None:
    """Reply 失败终态应保留任务，并明确说明没有可执行的清理步骤。"""

    holder: dict[str, _MultiImageGateway] = {}
    with pytest.raises(FlowExecutionError, match="FLOW_TERMINATED"):
    ...
    assert holder["gateway"].methods[-1] == "GetTask"
    assert "GetTaskResult" not in holder["gateway"].methods
    assert "DeleteTaskData" not in holder["gateway"].methods
    assert "当前 Flow 未配置可继续执行的终止清理步骤" in caplog.text
    assert "进入终止清理" not in caplog.text
...
def test_flow_runner_injects_one_stable_run_id_for_all_steps() -> None:
    """同一 Flow 的业务幂等键必须稳定，不能复用每次请求新建的 comm ID。"""
    ...
    class Gateway:
        ...
        def invoke(self, case: dict[str, object]) -> _Response:
            request = case["request"]
            assert isinstance(request, dict)
            params = self.context.resolve(request.get("params") or {})
            captured.append(str(params["client_request_id"]))
