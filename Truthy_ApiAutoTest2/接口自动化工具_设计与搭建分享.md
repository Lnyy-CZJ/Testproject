...
> 本文基于 `Truthy_ApiAutoTest2` 中的 MVP、V1.2、V1.3、Allure、Jenkins PRD与开发设计，以及 `test-platform/docs` 中的工具接入 PRD、开发计划和验收记录整理。本文聚焦现有 Gateway 接口自动化框架及其平台薄 Web 壳，不包含另一个独立项目 `api-test-agent` 的 AI 测试智能体方案。

## 1. 一句话介绍

这是一个面向 Gateway 业务的 YAML 数据驱动接口自动化工具：通过统一执行器读取 API 定义、单接口 Cases 和多接口 Flows，自动完成会话管理、参数传递、请求发送、分层断言、日志脱敏和报告生成；既可以在终端或 Jenkins 独立运行，也可以作为任务型工具接入测试开发平台。
...
| 阶段 | 核心问题 | 主要交付 |
| --- | --- | --- |
| MVP | 先证明配置驱动单接口执行可行 | YAML 用例、Gateway 请求构造、分层断言、终端脱敏输出 |
...
| Allure R1.0 | pytest 文本结果不便浏览和分享 | 业务化标题、有序步骤、脱敏附件、静态 HTML 报告 |
...
| HTTP 层 | `http_client.py` | Gateway POST、COS PUT、耗时记录和统一脱敏 |
| 断言层 | `assertions.py` | HTTP、Gateway、子响应及业务 data 分层断言 |
| 报告适配层 | `allure_reporter.py` | 动态标题、层级、步骤及脱敏附件 |
...
1. 调用前判断 access token 是否临期。
2. refresh token 有效时调用刷新接口。
3. 刷新失败或 refresh token 失效时重新创建匿名会话。
4. 新会话更新到当前运行时上下文。
5. 必要时将刷新后的会话写回 `.env`，供下一次独立执行复用。

这种方式避免每条测试重复处理登录逻辑，但也意味着凭证文件在运行环境中必须可写。平台接入时因此使用独立的 `.env.platform`，与开发者个人 `.env` 隔离。
...
日志和报告共同复用敏感信息规则。`auth_token`、`access_token`、`refresh_token`、`Authorization`、`session_token` 等字段显示为 `***`；预签名 URL 只记录 scheme、host 和 path，查询参数不落盘；上传文件只记录名称、Content-Type 和字节数，不记录二进制内容。

平台验收期间曾发现 Admin 接口在请求体中使用 `session_token`，原规则没有覆盖。项目以最小改动补充敏感键，并扫描历史产物和增加回归断言。这体现了安全规则需要以真实流量持续校正，而不是只依赖最初字段清单。
...
- Allure 原始结果及 HTML：供查看业务步骤和脱敏附件。
- 执行日志：供完整排查。
...
MVP 使用单执行槽位，不排队；已有任务运行时，新提交返回 `409 SLOT_BUSY`。任务记录保存在 `tasks/<task_id>.json`，控制台输出单独保存并再次脱敏。JSON 使用临时文件、fsync 和原子替换写入，避免页面轮询时读到半份记录。
...
- `data/` 以只读卷挂载，平台只能浏览和执行用例，不能在线修改。
- `.env.platform` 作为可写凭证文件挂载进容器，但不进入镜像、Git、接口响应或前端。
