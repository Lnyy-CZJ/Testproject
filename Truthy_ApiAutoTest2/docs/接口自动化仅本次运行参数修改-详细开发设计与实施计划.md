...
- 直接在 `/Users/admin/Testproject/Truthy_ApiAutoTest2` 当前本机工作目录实施，不创建新的 worktree；实施前后保留用户已有未提交改动，只暂存当前任务确认过的文件。
...
`runtime_overrides`、基础值和最终值始终留在 `Truthy_ApiAutoTest2`。
...
```text
API_AUTOTEST_EXECUTION_ASSET_FILE=/Users/admin/Testproject/Truthy_ApiAutoTest2/runtime/dating/<task_id>/execution-asset.json
```
